...
async function main() {
  ...
  const nftCreate = new TokenCreateTransaction()
    .setTokenName(process.env.TOKEN_NAME0)
    .setTokenSymbol(process.env.TOKEN_SYMBOL0)
    .setTokenType(TokenType.NonFungibleUnique)
    .setDecimals(0)
    .setInitialSupply(0)
    .setTokenMemo(process.env.TOKEN_METADATA_IPFS_URL0)
    .setTreasuryAccountId(treasuryId)
    .setSupplyType(TokenSupplyType.Finite)
    .setSupplyKey(treasuryKey)
    .setAdminKey(treasuryKey)
    .setFeeScheduleKey(treasuryKey)
    ...
    .setCustomFees([new CustomRoyaltyFee({
      ...
      numerator: process.env.TOKEN_ROYALTY_FEE_NUMERATOR0,
      denominator: process.env.TOKEN_ROYALTY_FEE_DENOMINATOR0,
      allCollectorsAreExempt: false,
    })])
    .setMaxSupply(process.env.TOKEN_MAX_SUPPLY0)
