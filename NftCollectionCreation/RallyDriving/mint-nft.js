...
const metadataLinks = require('../CollectionLinks.json');
...
const tokenId = process.env.RallyDriving_TokenId;
...
async function main() {
  let CID = metadataLinks["RallyDriving"].map(link => Buffer.from(link));
  while (CID.length) {
