...
import {
  ...
  NftClaimResponse,
  NftClaimRequest,
  CreateNftRequest, CreateNftResponse
...
export const blockchainApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getNftClaim: builder.mutation<NftClaimResponse, NftClaimRequest>({
      query: ({ accountId }) => ({
        url: `nft-claim/${accountId}`,
        method: "GET",
      }),
    }),
    
    createFreeNft: builder.mutation<CreateNftResponse, CreateNftRequest>({
      query: (data: CreateNftRequest) => ({
        url: 'create_free_nft',
        method: "POST",
        body: data,
      }),
...
export const {
  ...
  useGetNftClaimMutation,
  useCreateFreeNftMutation
} = blockchainApi;
