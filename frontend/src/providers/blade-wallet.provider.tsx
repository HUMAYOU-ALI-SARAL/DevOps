'use client'
import React, { PropsWithChildren, createContext, useContext, useLayoutEffect, useState , useEffect} from "react";
import { HederaNetwork, BladeWalletError } from '@bladelabs/blade-web3.js';
import {
...
const BladeWalletProvider = ({ children }: PropsWithChildren) => {
  ...
  const [localAccountPrivateKey, setLocalAccountPrivateKey] = useState<string>("");

  const trashCollectorContractId = "0.0.72048";
  const marketContractId = "0.0.3418318";

  // let hederaClient: Client | undefined; // Explicitly declare the type of hederaClient
  // let operatorKey: PrivateKey | undefined; // Explicitly declare the type of operatorKey
  ...
  // useLayoutEffect(() => {
    ...
    if (localWalletAccountId && localWalletAccountPrivateKey) {
      setLocalAccountPrivateKey(localWalletAccountPrivateKey);
      setLocalAccountID(localWalletAccountId);
    }
  }, []);
  ...
  const populateAndRunTx = async (tx: Transaction): Promise<string> => {
    if (!localAccountID || !localAccountPrivateKey) {
      throw new Error("Account ID or Private Key is not set");
    }
  
    const operatorKey = PrivateKey.fromStringDer(localAccountPrivateKey);
    const hederaClient = process.env.NEXT_PUBLIC_HEDERA_NETWORK === 'testnet'
      ? Client.forTestnet().setOperator(AccountId.fromString(localAccountID), operatorKey)
      : Client.forMainnet().setOperator(AccountId.fromString(localAccountID), operatorKey);
