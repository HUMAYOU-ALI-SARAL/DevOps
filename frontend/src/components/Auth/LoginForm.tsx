...
const LoginForm = () => {
  ...
  useEffect(() => {
    if (isSuccess && data) {
      ...
      if (data.dailyBonus) {
        localStorage.setItem("showSpointModal", "true");
      }
    ...
    <div className={`${styles.authForm} shadow rounded-lg`}>
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className={`${styles.paraLink}`}>
          ...
          />
          <div className={`text-red-500 h-[24px] ${formik.errors.email && formik.touched.email ? "visible" : "invisible"}`}>
            {formik.errors.email}
          </div>
        </div>
        <div className={`${styles.paraLink}`}>
      ...
      <p className={`${styles.paraLink}`} style={{ marginTop:'-15px' }}>
        {t("Do you check invite code")}?{" "}
        <Link className="text-orange underline" href="/auth/invite">
          {t("check invite code")}
        </Link>
      </p>
    </div>
